...
CREATE POLICY "공연 이미지 공개 읽기" ON storage.buckets FOR
SELECT
  USING (id = 'performances');

-- 인증된 사용자(또는 서비스 역할)가 파일을 올리고 지울 수 있게 허용
CREATE POLICY "공연 데이터 관리자 권한" ON storage.buckets FOR ALL TO authenticated USING (id = 'performances')
WITH
  CHECK (id = 'performances');