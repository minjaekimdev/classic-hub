...
const Field = ({ field, onFilterFieldClick }: FieldProps) => {
  ...
  return (
    <div
      className="grow shrink-0 flex justify-center items-center text-xs cursor-pointer"
      onClick={(e) => {
        e.stopPropagation();
        openField(field);
...
const FilterDesktopSmall = ({
  ...
  return (
    ...
    >
      <div
        className="grow-2 shrink-0 flex justify-center items-center ml-2 h-full text-xs cursor-pointer"
        onClick={(e) => {
          e.stopPropagation();
          openField("검색어");
          onFilterFieldClick(true);
