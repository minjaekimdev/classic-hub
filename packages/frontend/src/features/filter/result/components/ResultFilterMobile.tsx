        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white z-50 rounded-t-2xl shadow-xl max-h-[90vh] flex flex-col transition-transform duration-300 transform translate-y-0">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-8 pb-24">
              {/* 2. 정렬 카테고리 */}
              <Sort
                selectedSort={states.selectedSort}
                handleSortChange={actions.handleSortChange}
              />
              {/* 3. 지역 · 공연장 카테고리 */}
              <LocationHall
                regionArray={MOCK_REGIONS}
                expandedRegion={states.expandedRegion}
                selectedVenues={states.selectedVenues}
                onToggleRegion={actions.toggleRegion}
                onToggleVenue={actions.toggleVenue}
              />
            </div>
